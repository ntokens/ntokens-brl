# nTokens BRL Token
The nTokens BRL Token contract is an ERC-20 compatible token. 
...
- `owner` - re-assign any of the roles.
...
Each `minter` has a `mintingAllowance`, which nTokens configures. The `mintingAllowance` is how many tokens that minter 
may issue, and as a `minter` issues tokens, its `mintingAllowance` declines. 

### Adding Minters
nTokens adds minters via the `configureMinter` method. When a minter is configured a `mintingAllowance` is specified, 
...
minting. When a `minter`'s allowance is low, nTokens can make another call to `configureMinter` to reset the 
`mintingAllowance` to a higher value.

### Removing Minters
nTokens removes minters via the `removeMinter` method. This will remove the `minter` from the list of `minters` and set 
...
nTokens blacklists an address via the `blacklist` method. The specified `account` will be added to the blacklist.

- Only the `blacklister` role may call `blacklist`.
- Blacklisting emits a `Blacklist(account)` event

### Removing a blacklisted address
nTokens removes an address from the blacklist via the `unblacklist` method. The specified `account` will be removed from the blacklist.
...
required to fix or mitigate the issue that caused nTokens to pause the contract.

### Pause
nTokens will pause the contract via the `pause` method. This method will set the paused flag to true.
...
### Unpause
nTokens will unpause the contract via the `unpause` method. This method will set the `paused` flag to false. 
...
The `owner` role has the ability to reassign all roles (including itself).
